...
sequenceDiagram
    ...
    Note over A: ① 安全过滤<br/>guard_query() — 注入检测+危险词扫描

    Note over A: ② 缓存查询<br/>Redis L1 → MySQL L2 → 语义L1.5<br/>均未命中，继续

    Note over A: ③ Self-RAG<br/>need_retrieve() → YES，需要检索

    Note over A: ④ 多轮指代消解<br/>改写追问为独立查询

    Note over A: ⑤ 智能路由<br/>决策树6维特征 → hybrid

    Note over A: ⑥ 混合检索(并行)<br/>·稠密向量(Milvus×2)<br/>·BM25全文<br/>·图谱检索(Neo4j)<br/>·RAPTOR摘要

    Note over A: ⑦ RRF融合 → Rerank精排 → MMR去冗余

    Note over A: ⑧ CRAG自纠错<br/>·correct → 继续<br/>·ambiguous → 邻域扩展<br/>·incorrect → 改写重检索<br/>→ 仍不足则 refused

    Note over A: ⑨ GraphRAG<br/>知识图谱结构化上下文

    Note over A: ⑩ Agent诊断(可选)<br/>ReAct循环: Thought→Tool→Observation<br/>≤6轮，调规程/图谱/案例/操作票

    Note over A: ⑪ LLM生成<br/>带引用编号[1][2][3]的结构化答案

    Note over A: ⑫ 三层引用校验<br/>1.编号范围 / 2.chunk存在性 / 3.NLI声明核验<br/>→ 不通过者替换为警示

    Note over A: ⑬ 后处理<br/>PII脱敏 + 证据溯源 + 高风险提取

    Note over A: ⑭ 写缓存<br/>Redis L1 + MySQL L2 + 语义L1.5

    Note over A: ⑮ 异步后台<br/>·持久化对话 / ·成本追踪<br/>·在线评测 / ·证据补全

    U->>F: 输入问题
    F->>A: POST /api/qa/answer/stream (SSE)
    Note over A: ——— 以上 15 步按序执行 ———
    A-->>F: SSE 流式: token → meta → done
    F-->>U: 渲染 Markdown + 引用卡片 + 评价按钮
```
